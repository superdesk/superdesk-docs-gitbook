...
Beside the Hamburger Menu, you’ll see the blue drop-down Desks menu. It displays the name of the desk you’re currently working on. Click the desk name to view all desks you’re assigned to. Desks can be configured by editors/managers to have different Dashboard setups depending on the needs of your team.

Changes made to a Dashboard on a Desk will be visible to all users assigned to that Desk. Widgets on the Dashboard of your Custom Workspace are visible only to you.
...
1. Click the Create icon in the top-right of the Dashboard pane.
2. Select the widget you want from the widget options window and click **ADD THIS WIDGET**.
3. Click **Insert this Widget** to place it on your Dashboard.
4. You can add multiple displays of the same widget type.
{% endstep %}
{% endstepper %}
...
* To add different clocks, click the Available Clocks tab and scroll through the time zones. When you click on a time zone, it will automatically add itself to the Your Clock tab.
* If you set more than three clocks, you will have to adjust the size of your widget to display them all at the same time.
* In the World Clock Configuration window, you can switch between digital and analog displays under the Your Clock tab.
* When you are satisfied with the clocks you have chosen, click the **Save** button at the bottom of the window.
...
{% tabs %}
{% tab title="Desks" %}
In the Monitoring settings window, under the Desks tab, you can select the Stages that you wish to view in your Monitoring widget. You can only select the Stages in Monitoring Settings associated with the current desk.

The only exception to this occurs if a Stage is marked for Global Read by an administrator in the Hamburger Menu > Settings > Desks, in the Stage options. Stages that are marked for Global Read can be added to any Monitoring window in any Desk. If a Stage is currently empty, the Monitoring widget will not show any information from that Stage.
{% endtab %}
...
In the second drop-down menu on the Ingest Stats Configuration window you can choose a colour scheme for your Ingest Stats graph. The choice of colour is purely cosmetic.

Once you have adjusted your Ingest widget settings to your liking, click the **SAVE** button.

The information on the Ingest Widget is presented as a graph. In the source example below, you can see that the majority of the ingested articles are coming from Forbes.
...
* Items currently locked by that user.
* Items marked for that user.
* Items that have been created by that user.
* Items that have been moved to a working stage by that user.
...
In the example of a User Activity Widget shown above, the only activity that the displayed user has done is create a new item and attach photos to it. Each uploaded photo appears as a separate entry in this case.

#### Desk Routing and Other Widgets

Your Superdesk instance may contain widgets other than those described in this chapter. You can read about the Desk Routing widget in a subsequent chapter because of its more complex nature. For information about further widgets, please consult the documentation for your specific Superdesk instance.

<details>

<summary>Desk Routing widget</summary>

(Refer to the Desk Routing section in your documentation for more details.)
