...
{% code title="docker-compose deployment" %}
```bash
...
{% code title="kubernetes (placeholder)" %}
```bash
...
For detailed manual installation instructions, see the [Installation Guide](file:///docs/getting-started/installation).
...
{% code title="settings.py (logging)" %}
```python
# settings.py
import logging
...
* Database maintenance, update packages
{% endstep %}

{% step %}
### Quarterly

* Security audit, capacity planning
{% endstep %}
{% endstepper %}

### Update Process

{% code title="update process" %}
```bash
# Backup first
./backup.sh
...
* Review [Installation Guide](file:///docs/getting-started/installation)
* Check [GitHub Issues](https://github.com/superdesk/superdesk)
* Contact the Superdesk team

## Next Steps

* [Getting Started](file:///docs/getting-started/intro)
* [Architecture Overview](file:///docs/developer-guide/architecture)
* [Contributing](file:///docs/contributing)
